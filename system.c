...
/*------------------ Random ------------------*/
uint8_t z_random_u8(void) { return (uint8_t)random(); }

uint16_t z_random_u16(void) { return (uint16_t)random(); }

uint32_t z_random_u32(void) { return (uint32_t)random(); }
...
unsigned long z_time_elapsed_us(z_time_t *time) {
    ...
    unsigned long elapsed = (1000000 * (now.tv_sec - time->tv_sec) + (now.tv_nsec - time->tv_nsec) * 1000);
    return elapsed;
}
...
unsigned long z_time_elapsed_ms(z_time_t *time) {
    ...
    unsigned long elapsed = (1000 * (now.tv_sec - time->tv_sec) + (now.tv_nsec - time->tv_nsec) / 1000000);
    return elapsed;
}
